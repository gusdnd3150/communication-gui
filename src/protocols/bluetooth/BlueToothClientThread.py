...
class BlueToothClientThread(threading.Thread, Client):
    ...
    def __init__(self, data):
        ...
        #  'SK_TYPE': 'BLUETOOTH', 'SK_CLIENT_TYPE': 'KEEP', 'HD_ID': 'HD_FREE', 'SK_PORT': 5556, 'SK_IP': '0.0.0.0',
    ...
    async def scan_ble_devices(self):
        logger.info(f'bluetooth scan start')
        # BLE 장비 스캐너 초기화
    ...
    def initClient(self):
        ...
        try:
            ...
            self.logger.info('BLUETOOTH CLIENT Start : SK_ID={}, IP={}, PORT={}'.format(self.skId, self.skIp, self.skPort))
            ...
            with self.socket:
                while self.socket:
                    try:
                        ...
                        self.logger.error(f'BLUETOOTH CLIENT SK_ID={self.skId}  exception : {traceback.format_exc()}')
                        self.isRun = False
                        break
        except Exception as e:
            self.isRun = False
            self.logger.error(f'BLUETOOTH CLIENT SK_ID={self.skId}  exception : {e}')
