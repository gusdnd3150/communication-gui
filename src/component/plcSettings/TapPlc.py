...
class TapPlc():
    ...
    def setEvent(self):
        print(type(self.ui.list_plc))
        self.ui.list_plc.cellClicked.connect(self.selectRow)
        self.ui.list_plc.currentCellChanged.connect(self.selectRow)
        #
        # self.ui.list_plc.setEnabled(True)
        # self.ui.list_plc.currentCellChanged.connect(self.selectRow)
        # self.ui.list_plc.itemChanged.connect(self.selectRow)
        # 소켓 탭 이벤트 설정
    ...
    def createGrid(self):
        try:
            headers = ['PKG_ID','PLC_ID','USE_YN','PLC_MAKER','CPU_TY','SLOT','RACK','PLC_IP','PLC_PORT','SK_LOG','COMM_TY','LOG_YN']
            ...
            self.ui.list_plc.setColumnCount(12)
    ...
    def selectRow2222(self,row, column):
        logger.info('ssss')
        try:
            self.contFlag = 'upd'
            row_data = {}
            ...
            #
            # self.ui.PLC_PLC_USE_YN.setCurrentText(row_data['USE_YN'])
